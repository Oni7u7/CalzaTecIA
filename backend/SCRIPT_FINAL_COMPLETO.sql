-- =====================================================
-- Script SQL FINAL COMPLETO para crear todas las tablas en Supabase
...
-- Corrige automáticamente columnas incorrectas si existen
-- =====================================================
...
-- =====================================================
-- CORRECCIÓN PREVIA: Renombrar numero_ticket a ticket si existe
-- =====================================================

DO $$ 
BEGIN
  -- Verificar si existe la columna numero_ticket en ventas
  IF EXISTS (
    SELECT 1 
...
-- Tabla: ventas
-- IMPORTANTE: Usa 'ticket' no 'numero_ticket'
...
-- Si la tabla ya existe pero tiene numero_ticket, agregar columna ticket si no existe
DO $$ 
BEGIN
  IF EXISTS (
    ...
    -- Si no existe la columna ticket, agregarla
    IF NOT EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_name = 'ventas' 
      AND column_name = 'ticket'
    ) THEN
      -- Si existe numero_ticket, ya fue renombrada arriba
      -- Si no existe ninguna, agregar ticket
      ALTER TABLE ventas ADD COLUMN ticket VARCHAR(50);
      -- Hacerla UNIQUE si no hay datos
      ALTER TABLE ventas ADD CONSTRAINT ventas_ticket_unique UNIQUE (ticket);
    END IF;
  END IF;
END $$;
...
-- ✅ La columna numero_ticket se renombra a ticket si existe
-- 
-- PRÓXIMOS PASOS:
