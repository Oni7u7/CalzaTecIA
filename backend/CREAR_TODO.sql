-- ============================================
-- SCRIPT COMPLETO - CREAR TODO LO NECESARIO
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'ℹ️ Tabla solicitudes_registro ya existe';
    
    -- Asegurarse de que RLS esté desactivado
    ALTER TABLE solicitudes_registro DISABLE ROW LEVEL SECURITY;
  END IF;
END $$;
...
SELECT 
  '=== VERIFICACIÓN DE USUARIOS ===' as verificacion;
...
SELECT 
  '=== VERIFICACIÓN DE TABLA SOLICITUDES ===' as verificacion;
...
SELECT 
  CASE 
    WHEN EXISTS (
      ...
      WHERE table_schema = 'public' 
      AND table_name = 'solicitudes_registro'
    ) THEN '✅ Tabla solicitudes_registro existe'
    ELSE '❌ Tabla solicitudes_registro NO existe'
  END as estado;

-- Verificar RLS
SELECT 
  '=== VERIFICACIÓN DE RLS ===' as verificacion;
