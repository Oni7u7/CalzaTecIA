...
import { UserCheck, Mail, Calendar, Shield, AlertCircle, CheckCircle, XCircle, RefreshCw, LogOut } from 'lucide-react'
...
export default function ModeradorPage() {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          aprobado_por: usuario.id,
          fecha_aprobacion: new Date().toISOString(),
          notas: motivo || 'Solicitud rechazada por el moderador'
        })
        .eq('id', solicitudId)
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--color-neutral-950)' }}>
      {/* Header */}
      <div className="border-b" style={{ borderColor: 'var(--color-neutral-800)', background: 'var(--color-neutral-900)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: 'var(--gradient-primary)' }}>
                <UserCheck className="w-6 h-6" style={{ color: 'var(--color-neutral-925)' }} />
              </div>
              <div>
                <h1 className="text-2xl font-extrabold" style={{ color: 'var(--color-neutral-100)' }}>
                  Panel de Moderador
                </h1>
            ...
            <div className="flex items-center gap-4">
              {usuarioActual && (
                <div className="text-right">
                  <p className="text-sm font-bold" style={{ color: 'var(--color-neutral-100)' }}>{usuarioActual.nombre}</p>
                  <p className="text-xs" style={{ color: 'var(--color-neutral-300)' }}>Moderador</p>
                </div>
              )}
              <Button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Card style={{ background: 'var(--color-neutral-900)', border: '1px solid var(--color-neutral-800)' }}>
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-semibold mb-1" style={{ color: 'var(--color-neutral-300)' }}>
                      Pendientes
                    </p>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-extrabold" style={{ color: 'var(--color-green)' }}>
                      {solicitudes.filter(s => s.estado === 'aprobada').length}
                    </p>
                  </div>
                  <CheckCircle className="w-8 h-8" style={{ color: 'var(--color-green)' }} />
                </div>
              </CardContent>
            </Card>
            <Card style={{ background: 'var(--color-neutral-900)', border: '1px solid var(--color-neutral-800)' }}>
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-extrabold" style={{ color: 'var(--color-neutral-100)' }}>
                      {solicitudes.length}
                    </p>
                  </div>
                  <UserCheck className="w-8 h-8" style={{ color: 'var(--color-teal-400)' }} />
                </div>
              </CardContent>
            </Card>
          </div>
